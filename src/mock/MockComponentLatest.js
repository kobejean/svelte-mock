...
function create_fragment(ctx) {
	...
	const slot_template = /* $$slots */ ctx[ctx.length - 1];
	const slots = {}
	for (const slot_name of Object.keys(slot_template)) {
		slots[slot_name] = create_slot(slot_template[slot_name], ctx, /* $$scope */ ctx[ctx.length - 2]);
	}
	...
	return {
		...
		p(ctx, [dirty]) {
			for (const [name, slot] of Object.entries(slots)) {
				if (slot && slot.p && changed.$$scope && dirty & /*$$scope*/ 1) {
					slot.p(
						get_slot_context(slot_template[name], ctx, /*$$scope*/ ctx[ctx.length - 2], null),
						get_slot_changes(slot_template[name], ctx, /*$$scope*/ ctx[ctx.length - 2], dirty, null)
					);
				}
			}
		},
